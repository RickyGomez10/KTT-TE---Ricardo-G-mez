...
router.get('/america/top20', async function(req, res, next){
    ...
    res.render('topAmerica', {resultado: response, ttitle: americaContentTitle, america: 'active', europe: '', africa: '', dashboard: ''});
})

router.get('/europe/top20', async function(req, res, next){
    response = await CountryRepository.GetCountriesByRegion("Europe");
    res.render('topEurope', {resultado: response, ttitle: europeContentTitle, america: '', europe: 'active', africa: '', dashboard: ''});
})

router.get('/africa/arabic', async function(req, res, next){
    response = await CountryRepository.GetCountriesByRegion("Africa");
    res.render('arabicCountries', {resultado: response, ttitle: africaContentTitle, america: '', europe: '', africa: 'active', dashboard: ''});
})
...
router.get('/country/:countryName', async function(req, res, next){
    ...
    res.render('countryDetail', {resultado: country, america: '', europe: '', africa: '', dashboard: ''})
})
router.get('/dashboard', async function(req, res, next){
    ...
    res.render('dashboard', {americaCountries: americaCountries.slice(0, dashBoardAmericaTop),europeCountries: europeCountries.slice(0, dashBoardEuropeTop),africaCountries: africaCountries,africaLanguages:africaLanguages.slice(0,dashBoardAfricaLanguagesTop), america: '', europe: '', africa: '', dashboard: 'active'})
})
